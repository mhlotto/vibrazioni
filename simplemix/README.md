...
- `linkedin-view-tracker`: Track LinkedIn profile-viewer snapshots in a JSONL file.
- `pickcols`: Read stdin and print selected whitespace fields joined by a separator.
- `ssh-loaded-keys`: Print `~/.ssh` key filenames loaded in `ssh-agent`; use `--with-key` to include the public key.
